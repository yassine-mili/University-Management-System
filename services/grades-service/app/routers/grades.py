...
def generate_transcript(student_id: int, db: Session = Depends(get_db)):
...
@router.get(
    "/statistics/distribution/{student_id}",
...
def get_grade_distribution(student_id: int, db: Session = Depends(get_db)):
